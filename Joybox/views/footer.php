<img src="../JoyBox/img/FOOTER.png" class="img-footer">
<footer class="c_footer text-light py-3">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <h5>Sobre nosotros</h5>
                <p>Somos un pasionado equipo de viajeros que le encanta crear y planificar tus proximos lugares.</p>
            </div>
            <div class="col-md-4">
                <h5>Contacto</h5>
                <p>Email: Viajesneco26@gmail.com</p>
                <p>Teléfono: +54 9 11 XXXX-XXXX</p>
            </div>
            <div class="col-4 container p-0" id="redes_menu">
                <div class="row redes-f">
                    <div class="redes-ftr">
                        <div class="col-md-4 f-footer" id="redes-ar">
                            <div class="col-1 p-0 redes_menu bi bi-instagram"></div>
                            <div class="col-1 p-0 redes_menu bi bi-tiktok"></div>
                            <div class="col-1 p-0 redes_menu bi bi-twitter"></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <hr class="footer">
    <div class="row copyright">
        <div class="col-md-4 f-footer">
            <p class="text-center">&copy; 2023 Viajesneco. All rights reserved.</p>
        </div>
        <div class="col-md-4 f-footer" id="TyP">
            <a class="f-text" href="polpri.php">Privacy Policy</a>
            <a class="f-text" href="tercon.php">Terms and Conditions</a>
        </div>
        <div class="col-md-4 f-footer" id="redes">
            <a href="https://matias.ma/nsfw/" target="_blank" class="col-1 p-0 redes_menu bi bi-instagram"></a>
            <a href="https://www.tiktok.com/@gamewise_et26?lang=es" target="_blank" class="col-1 p-0 redes_menu bi bi-tiktok red"></a>
            <a href="https://twitter.com/GameWise_et26"  target="_blank" class="col-1 p-0 redes_menu bi bi-twitter"></a>
        </div>
    </div>
    </div>
</footer>